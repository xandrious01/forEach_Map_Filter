...
function showFirstAndLast(arr){
    const newArr = arr.map(string => string.substr(0,1).concat(string.slice(-1)));
  return newArr;
}
...
function vowelCount(str){
    ...
    let count = {};
    const onlyVowels = (strArr.filter(letter => 
      vowelsArr.includes(letter)).forEach(vowel=>{
      if (count[vowel]) {
        count[vowel] += 1} else {
          count[vowel] = 1}
      })
...
returns a new array with each value multiplied by the index 
...
function valTimesIndex(arr){
    const newArray = arr.map((value,index) => value*index);
    return newArray;
}
...
function extractKey(arr, key){
  const newArray = arr.map((obj)=>obj[key]);
  return newArray;
}
...
const newArray = arr.map(name => {return (name.first + ' ' + name.last)})
      return newArray;
}
...
function filterByValue(arr,key) { 
    const filtered = arr.filter((obj)=> Object.keys(obj).includes(key));
   return filtered; 
}
...
function find(arr, searchValue) {
    const newArr = arr.filter((value) => value === searchValue);
    return newArr[0];
}
...
function findInObj(arr, key, searchValue){
    const newArr = arr.filter(obj=> Object.keys(obj).includes(key) && obj[key]===searchValue)
  return newArr[0];
  }
...
let notVowels = strArray.filter((letter)=>(!(vowels.includes(letter)))).join('');
return notVowels;
}

/*
Write a function called doubleOddNumbers which accepts an array and 
returns a new array with all of the odd numbers doubled (HINT - you 
    can use map and filter to double and then filter the odd numbers).
...
function doubleOddNumbers(arr) {
    const oddsDoubled = (arr.filter((num)=>num%2 !== 0)).map(num=>num*2);
    return oddsDoubled;
}
